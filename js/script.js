...
function showProgressBar() {
  const progressBar = document.querySelector(".works-progress-bar"); // 예시로 선택한 클래스
  if (progressBar) {
    // progressBar가 null이 아닌지 확인
    progressBar.style.width = "100%"; // 진행 표시줄의 너비를 설정
  } else {
    console.error("Progress bar element not found");
  }
}

// 비디오 currentTime 설정 시, 해당 요소가 로드되었는지 확인
const videoElement = document.getElementById("modalVideo");
if (videoElement) {
  videoElement.currentTime = 0; // currentTime 설정
} else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  thumbItems.forEach((item) => {
    ...
    if (video) {
      ...
      // 비디오가 이미 준비되어 있는 경우 바로 로딩 아이콘 숨기기
      if (video.readyState >= 2) {
        ...
        video.addEventListener("error", () => {
          loader.style.display = "none"; // 에러가 나도 숨김
        });
      }
    }
  });
